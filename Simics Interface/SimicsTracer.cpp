...
public:
  ...
  API::cycles_t trace_mem_hier_operate(API::conf_object_t * space, API::map_list_t * map, API::generic_transaction_t * aMemTrans) {
    ...
    //Obtaining the value of store requests.
    if (theMemoryMessage.isWrite())
    {
      theMemoryMessage.reqSize() = mem_trans->s.size;
        theMemoryMessage.hasWordValue() = true;
        theMemoryMessage.accessTheValueOfWord() = (uint64_t)API::SIM_get_mem_op_value_cpu(aMemTrans);
    }
    //End of obtaining the value of store requests.
